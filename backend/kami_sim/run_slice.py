"""Vertical slice CLI runner — spec §3.2 Phase 3.

Usage: python -m kami_sim.run_slice --ticks 200
"""
...
from .slice_world import build_slice_world
...
async def run_slice(num_ticks: int = 200, output_file: str = "slice_output.json"):
    """Run the vertical slice simulation."""
    logger.info("=" * 60)
    logger.info("KAMI SIMULATION — VERTICAL SLICE")
    logger.info("=" * 60)

    # Initialize database (in-memory for slice)
    db_url = "sqlite:///./kami_slice.db"
    engine, session_factory = init_db(db_url)
    session = session_factory()

    # Build the test world
    logger.info("Building slice world: 5 kami, 3 agents, 7 objects...")
    spatial_graph = build_slice_world(session)
    session.close()
...
def main():
    parser = argparse.ArgumentParser(description="Run Kami Simulation vertical slice")
    parser.add_argument("--ticks", type=int, default=200, help="Number of ticks to simulate")
    parser.add_argument("--output", type=str, default="slice_output.json", help="Output JSON file")
    args = parser.parse_args()

    asyncio.run(run_slice(num_ticks=args.ticks, output_file=args.output))
